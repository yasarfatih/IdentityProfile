...
namespace IdentityProfileApp.Utils.Extensions
{
    ...
    {
        public static Image CropImage(this HttpPostedFileBase postedImage, int width, int height)
        {
            Image image = Image.FromStream(postedImage.InputStream);
            var croppedImage = image.GetThumbnailImage(width, height);
            return croppedImage;
        }
        ...
        public static string GenerateFileName(this string fileName)
        {
            ...
            string newFileName = $"{now.Day}, {now.Month}, {now.Year}, {now.Hour}, {now.Minute}, {now.Second}, {now.Millisecond}, {extension}";
            return newFileName;
        }
